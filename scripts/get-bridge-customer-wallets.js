...
if (!customerId) {
  ...
  console.error('Usage: node get-bridge-customer-wallets.js <customerID>');
  process.exit(1);
}
...
console.log(`Fetching wallets for customer ID: ${customerId}`);
...
axios
  ...
  .catch(err => {
    console.error('Error fetching customer wallets:');
    if (err.response) {
