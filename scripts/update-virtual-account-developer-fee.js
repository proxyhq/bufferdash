...
if (args.length < 3) {
  console.error('Usage: node update-virtual-account-developer-fee.js <customerID> <virtualAccountID> <developerFeePercent>');
  console.error('Example: node update-virtual-account-developer-fee.js cust_123 va_456 1.5');
  process.exit(1);
}

const customerID = args[0];
const virtualAccountID = args[1];
const developerFeePercent = args[2];
...
console.log(`Updating developer fee for Virtual Account ${virtualAccountID}...`);
...
axios
  ...
  .catch((error) => {
    console.error('Error updating virtual account developer fee:');
    if (error.response) {
